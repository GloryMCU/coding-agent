...
当前版本提供 `read_file`、`search_text`、`write_file`、`apply_patch` 和
`delete_file`。Agent 可以先定位并读取代码，再以受工作区沙箱约束的方式创建、
精确修改或删除文件。
...
`create_read_only_registry` 仍可供只读嵌入场景使用，CLI 默认使用包含全部五个工具的
...
下一阶段建议依次增加 `list_files` 和受控 Shell；历史量
明显增长且关键词召回不足后，再评估向量语义检索与混合排序。
